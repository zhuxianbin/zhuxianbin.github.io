<template>
  <div class="app-container">
    <div class="panel-container">
      <!--form表单-->
      <el-form :inline="true" :model="formTest" class="filter-container">
      ...
      <el-table :data="list">
        ...
        <el-table-column label="策略名称" width="200" prop="strategyTitle">
          <template slot-scope="scope">
            <span
              style="color: #1591FA;cursor: pointer;"
              @click="popSet(scope.row)"
            >{{scope.row.strategyTitle}}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="一级返佣" prop="firstRebateAmount">
          <template slot-scope="scope">
            <span>
              {{scope.row.firstRebateAmount ? scope.row.firstRebateAmount : 0}}元/
        ...
        <el-table-column label="二级返佣" prop="secondRebateAmount">
          <template slot-scope="scope">
            <span>
              {{scope.row.secondRebateAmount ? scope.row.secondRebateAmount : 0}}元/
...
<script>
import {
  getPartnerBonusDetailLongList,
  getPartnerBonusDetailList
} from "@/api/cdzRewardDetails";
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      getPartnerBonusDetailLongList(this.formTest)
        .then(res => {
