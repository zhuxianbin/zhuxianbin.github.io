<template>
  <div>
    <el-form :model="formTest" class="filter-container" :inline="true" label-width="120px">
    ...
    <el-table :data="list" style="margin-top: 20px">
      <el-table-column label="优惠券名称" width="150" prop="name">
        <template slot-scope="scope">
          <span
            class="portsn"
            style="color: #558FF2;cursor: pointer;"
      ...
      <el-table-column label="面值" width="200">
        <template slot-scope="scope">
          <span
            v-if="scope.row.couponsType == 'mj'"
          >满{{scope.row.fullAmount}}减{{scope.row.minusAmount}}</span>
          <span
            v-else-if="scope.row.couponsType == 'zk'"
          >{{scope.row.discount*10}}折，最高抵扣{{scope.row.discountLimit}}元</span>
        </template>
      </el-table-column>
      <el-table-column label="有效期" width="240">
        <template slot-scope="scope">
          ...
          <span
            v-else
          >{{scope.row.validStartTime|timeStr}} {{scope.row.useStartTime}} 至 {{scope.row.validEndTime|timeStr}} {{scope.row.useEndTime}}</span>
        </template>
      </el-table-column>
      <el-table-column label="所属公司" width="150" prop="company"></el-table-column>
      ...
      <el-table-column label="消耗金币" width="200">
        <template slot-scope="scope">
          <div style="width: 200px;">
            <el-input
              placeholder
              v-model="scope.row.bonusConsumeAmount"
              style="width: 130px;"
              :disabled="scope.row.noUse"
            ></el-input>
            <i
              class="el-icon-edit"
              style="font-size: 16px;color: black;cursor: pointer;"
              v-show="scope.row.noUse"
              @click="editRow(scope.row)"
            ></i>
            <i
              class="el-icon-success"
              style="font-size: 16px;color: green;cursor: pointer;"
              v-show="!scope.row.noUse"
              @click="affirmRow(scope.row)"
            ></i>
          </div>
