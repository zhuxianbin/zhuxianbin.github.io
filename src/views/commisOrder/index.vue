      ...
      <el-table :data="list" border>
        ...
        <el-table-column label="操作" width="150">
          <template slot-scope="scope">
            ...
            <template v-if="partner.permission.authority['HHR_COMMISSION_SETTLEMENT_ORDER'].includes('HHR_ORDER_AUDIT')">
              <el-button type="text" @click="reviewFun(scope.row)" v-if="scope.row.state == 0" style="color: #6440F2">订单审核</el-button>
            </template>
            
            <template v-if="partner.permission.authority['HHR_COMMISSION_SETTLEMENT_ORDER'].includes('HHR_INVOICE_AUDIT')">
              <el-button type="text" @click="reviewFun(scope.row)" v-if="scope.row.state == 1" style="color: #DBA54F">发票审核</el-button>
            </template>
            
            <template v-if="partner.permission.authority['HHR_COMMISSION_SETTLEMENT_ORDER'].includes('HHR_FINANCIAL_PAYMENT')">
              <el-button type="text" @click="reviewFun(scope.row)" v-if="scope.row.state == 2" style="color: #5CCD42">支付确认</el-button>
            </template>
            <template v-if="partner.permission.authority['HHR_COMMISSION_SETTLEMENT_ORDER'].includes('HHR_SETTLEMENT_EXPORT')">
...
export default {
  ...
  methods: {
    ...
    firstGetCommissionSettlementOrder() {
      if (this.partner.permission.authority['HHR_COMMISSION_SETTLEMENT_ORDER'].includes('HHR_ORDER_AUDIT')) {
        // 当有订单审核权限的时候
        this.formTest.state = "0";
      } else if (this.partner.permission.authority['HHR_COMMISSION_SETTLEMENT_ORDER'].includes('HHR_INVOICE_AUDIT')) {
        // 当有发票审核权限的时候
        this.formTest.state = "1";
      } else if (this.partner.permission.authority['HHR_COMMISSION_SETTLEMENT_ORDER'].includes('HHR_FINANCIAL_PAYMENT')) {
        // 支付确认权限的时候
