<template>
  <div class="app-container">
    <div class="panel-container">
      <!--查询的功能-->
      <el-form :model="formTest" class="filter-container" :inline="true">
        ...
        <el-form-item label="车辆类型">
          <el-select v-model="formTest.carType" clearable placeholder="选择车辆类型">
            <el-option v-for="item in carTayes" :key="item.id" :label="item.name" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="上次完成保期">
          <el-select v-model="formTest.maintainPeriod" clearable placeholder="上次完成保期">
            <el-option
              v-for="item in maintainPeriods"
              :key="item.value"
              :label="item.name"
              :value="item.value"
            ></el-option>
      ...
      <div class="xlsx-box">
        <el-button
          v-if="partner.permission.authority['HHR_MAINTAIN_MONITOR'].includes('HHR_MAINTENANCE_OUTPUT')"
          type="success"
          class="btn2"
          @click="leadOut"
          :loading="downLoading"
      ...
      <el-table :data="list" style="margin-top: 20px;">
        ...
        <el-table-column label="注册日期" width="150" prop="registerTimeStr"></el-table-column>
        <el-table-column label="上次保养时间" width="150" prop="lastMaintainTimeStr"></el-table-column>
        ...
        <el-table-column label="操作" width="210">
          <template slot-scope="scope">
            <el-button
              v-if="partner.permission.authority['HHR_MAINTAIN_MONITOR'].includes('HHR_ADD_VEHICLE_MAINTAIN_RECORD')"
              type="text"
              style="color: #0C7D9D"
              @click="newLog(scope.row)"
            >新增记录</el-button>
    ...
    >
      <new-log
        v-if="diaObj.diaIndex == 0"
        v-on:refeClose="refreshList"
        v-on:closeDialog="exitDialog"
        :data-row="newDate"
      ></new-log>
...
export default {
  ...
  methods: {
    ...
    leadOut() {
      ...
      turnForm.action =
        process.env.BASE_API + "/api/maintenance/maintenanceOutput";
    ...
    getList() {
      ...
      getVehicleMaintainList(this.formTest)
        .then(res => {
          this.list = res.list;
          if (this.list.length > 0) {
            this.list.forEach(ele => {
              ele.totalMileage = (ele.totalMileage * 1).toFixed(2);
              ele.lastMaintainMileage = (ele.lastMaintainMileage * 1).toFixed(
                2
              );
            });
          }
    ...
    newLog(row) {
      ...
      obj.lastMaintainPeriodName = this.maintainPeriods[
        obj.lastMaintainPeriod * 1
      ].name;
      this.newDate = obj;
