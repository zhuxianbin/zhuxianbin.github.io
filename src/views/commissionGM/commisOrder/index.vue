    ...
    <div class="panel-container">
      <el-form :model="formTest" class="filter-container" :inline="true">
        ...
        <el-form-item label="选择时间">
          <el-date-picker v-model="formTest.time" type="month" align="right" value-format="yyyy-MM"></el-date-picker>
        </el-form-item>
      ...
      <el-table :data="list">
        ...
        <el-table-column label="操作" width="150">
          <template slot-scope="scope">
            ...
            <template
              v-if="partner.permission.authority['HHR_COMMISSION_SETTLEMENT_ORDER'].includes('HHR_ORDER_AUDIT')"
            >
              <el-button
                type="text"
                @click="reviewFun(scope.row)"
            ...
            <template
              v-if="partner.permission.authority['HHR_COMMISSION_SETTLEMENT_ORDER'].includes('HHR_INVOICE_AUDIT')"
            >
              <el-button
                type="text"
                @click="reviewFun(scope.row)"
            ...
            <template
              v-if="partner.permission.authority['HHR_COMMISSION_SETTLEMENT_ORDER'].includes('HHR_FINANCIAL_PAYMENT')"
            >
              <el-button
                type="text"
                @click="reviewFun(scope.row)"
            ...
            <template
              v-if="partner.permission.authority['HHR_COMMISSION_SETTLEMENT_ORDER'].includes('HHR_SETTLEMENT_EXPORT')"
            >
...
export default {
  ...
  methods: {
    ...
    firstGetCommissionSettlementOrder() {
      if (
        this.partner.permission.authority[
          "HHR_COMMISSION_SETTLEMENT_ORDER"
        ].includes("HHR_ORDER_AUDIT")
      ) {
        // 当有订单审核权限的时候
        ...
        this.partner.permission.authority[
          "HHR_COMMISSION_SETTLEMENT_ORDER"
        ].includes("HHR_FINANCIAL_PAYMENT")
      ) {
        // 支付确认权限的时候
    ...
    exportData(row) {
      ...
      getCommissionSettlementOrderDetai(formSerach).then(res => {
        ...
        // 创建二进制对象写入转换好的字节流
        const tmpDown = new Blob([this.s2ab(XLSX.write(wb, wopts))], {
