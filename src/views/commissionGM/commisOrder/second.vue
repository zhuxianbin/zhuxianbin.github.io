      ...
      <el-table :data="list">
        ...
        <el-table-column label="操作" width="150">
          <template slot-scope="scope">
            ...
            <template
              v-if="partner.permission.authority['HHR_COMMISSION_AUDIT'].includes('HHR_ORDER_AUDIT_COPY')"
            >
              <el-button
                type="text"
            ...
            <template
              v-if="partner.permission.authority['HHR_COMMISSION_AUDIT'].includes('HHR_INVOICE_AUDIT_COPY')"
            >
              <el-button
                type="text"
                @click="reviewFun(scope.row)"
            ...
            <template
              v-if="partner.permission.authority['HHR_COMMISSION_AUDIT'].includes('HHR_FINANCIAL_PAYMENT_COPY')"
            >
              <el-button
                type="text"
                @click="reviewFun(scope.row)"
            ...
            <template
              v-if="partner.permission.authority['HHR_COMMISSION_AUDIT'].includes('HHR_SETTLEMENT_EXPORT_COPY')"
            >
...
export default {
  ...
  methods: {
    ...
    firstGetCommissionSettlementOrder() {
      if (
        this.partner.permission.authority["HHR_COMMISSION_AUDIT"].includes(
          "HHR_ORDER_AUDIT_COPY"
        )
      ) {
        // 当有订单审核权限的时候
